...
export const TREND_REVIEWER_AGENTS = [
  ...
  {
    id: 'official_landing_optimizer',
    name: '공식 오피셜 출처 & 스마트 랜딩 최적화관',
    role: '공식 직통 랜딩(공식몰/예약처/공지)의 채택 여부만 검증하는 오피셜 출처 전담 위원',
    scope: '오직 "출처의 공식성"만: 포털 검색결과 대신 브랜드 공식몰/공식 예약처/공식 공지 직통 링크 채택 여부',
    forbidden: '링크의 작동 여부(랜딩 팩트체커 관할), CTA 카피는 절대 채점 금지',
    penalties: [
      '🏛️ 공식 오피셜 직통 카드가 본문에 없으면 -4점',
      '공식 출처(공식몰/예약처)가 존재하는 주제인데 포털 검색결과 링크로만 대체되어 있으면 -3점',
      '공식 발표 내용과 비공식 커뮤니티 소문이 구분 없이 섞여 있으면 -2점',
    ],
  },
];
...
${a.penalties.map((p) => `     · ${p}`).join('\n')}`
  ...
  const prompt = `당신은 대한민국 최고의 트렌드/미디어 바이럴 16인 감수 위원회 시뮬레이터입니다.
아래 트렌드 원고(Round ${round} 버전)를 16인 각자의 관점에서 **독립적으로** 채점하세요.
...
16인 감수 위원회의 피드백(Round ${round})을 반영하여 기존 원고를 전면 리라이팅하세요.

[피드백 반영 우선순위 및 충돌 조정 규칙 — 절대 준수]
1. **감점 지시 100% 우선 반영**: "필수 반영 지시" 목록은 우선순위(낮은 점수) 순이다. 위에서부터 하나도 빠짐없이 본문에 반영한다.
2. **충돌 시 서열**: ① 팩트/링크/고지 문구 정정 > ② 필수 구성 요소 추가(단점 문단·FAQ·가격표·공식 카드) > ③ 구조/레이아웃(문단 분리·요약 박스·임베드 위치) > ④ 제목/카피 표현. 하위 서열 지시가 상위 서열 지시와 충돌하면 상위를 따른다.
3. **강점 보존**: 8점 이상 위원이 칭찬한 요소는 삭제/훼손하지 말고 유지한다.
4. **필수 골격 불변**: 3줄 요약 박스, 🏛️ 공식 오피셜 직통 카드, 내돈내산 단점 문단, FAQ 3선, 유튜브 임베드, (쇼핑 주제) 쿠팡 CTA + 공정위 고지 문구는 어떤 경우에도 삭제 금지. 없다면 이번 리라이팅에서 반드시 생성한다.
5. **링크 창작 금지**: 아래 "사전 검증 링크 목록"에 있는 URL만 사용한다. 새 URL을 임의로 만들지 않는다.
6. **모바일 최적화**: 모든 문단 2~3문장, 핵심 단어 <strong>, 표는 overflow-x 스크롤 래퍼.
...
export async function executeTwoRoundTrendReviewLoop(
  ...
  console.log('\n================================================================');
  console.log('🏛️ [스킬 가동] 트렌드 16인 전문 위원회 감수 루프 시작 (최소 2회 + 75점 돌파제)');
  console.log('================================================================');
...
16인 콘텐츠 감수 위원회와 5인 엔지니어링 감사를 모두 통과한 원고에 대해 "최종 발행 승인 폴리싱"만 수행하세요.

[편집국장의 권한과 한계 — 절대 준수]
1. **폴리싱 전용**: 새 팩트/새 링크/새 가격을 창작하지 않는다. 이미 감수된 링크·가격·주소·고지 문구는 절대 변경 금지.
2. **삭제 금지 골격**: 3줄 요약 박스, 🏛️ 공식 오피셜 직통 카드, 내돈내산 단점 문단, FAQ 3선, 유튜브 임베드, 쿠팡 CTA + 공정위 고지 문구는 반드시 그대로 보존한다.
3. **허용 작업 (오직 이것만)**:
   - 문맥 리듬감 & 톤앤매너 통일: 16인의 개별 수정 사항이 이질감 없이 하나의 글로 읽히도록 연결부만 다듬기
