...
async function runTrendPipeline() {
  ...
  for (let candidateIdx = 0; candidateIdx < candidateTopics.length; candidateIdx++) {
    ...
    // --- 2단계: 표준 키워드 정제 & 멀티모달(DOM+Vision) 랜딩 전수 검증 ---
    console.log('\n[2단계] 표준 키워드 정제 및 Playwright 4대 랜딩(공식처·네이버·유튜브·쿠팡) 전수 검증...');
      ...
      if (telegramBotToken && telegramChatId) {
        ...
        if (nextCandidate) {
          await telegram.sendMessage(
            ...
            `❌ <b>반려 주제:</b> ${topic.keyword} (${finalScore}점 / 기준: 80점)\n` +
            `🔄 <b>감수 이력:</b> ${reviewSummary}\n\n` +
            `🚀 <b>자동 조치:</b> 80점을 넘지 못해 즉시 차순위 대세 후보 [<b>${nextCandidate.keyword}</b>] 로 전환하여 고품질 원고 재탐구를 시작합니다!`
          );
        } else {
          await telegram.sendMessage(
  ...
  if (publishedSuccess) {
    console.log('\n================================================================');
    console.log('🎉 [트렌드 파이프라인 완료] 고품질 검증을 통과한 포스트가 성공적으로 완료되었습니다!');
    console.log('================================================================\n');
  } else {
    console.log('\n⚠️ [트렌드 파이프라인 종료] 기준(80점)을 만족하는 후보가 없어 안전하게 중단되었습니다.\n');
  }
}
