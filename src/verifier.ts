...
export function auditAndFixHtmlLinks(
  ...
  // 2. 텍스트 이미지 플레이스홀더 / 빈 회색 박스 100% 완전 삭제 (불필요한 빈공간 원천 차단)
  fixedHtml = fixedHtml.replace(/<!--[\s\S]*?이미지[\s\S]*?-->/gi, '');
  fixedHtml = fixedHtml.replace(/<div[^>]*>[\s\S]*?(📸|\[이미지:|Alt:|이미지 가이드|포토존)[\s\S]*?<\/div>/gi, '');
  fixedHtml = fixedHtml.replace(/📸\s*\[이미지:[^\]]*\]/gi, '');
  fixedHtml = fixedHtml.replace(/<p[^>]*>[\s\S]*?📸[\s\S]*?<\/p>/gi, '');
